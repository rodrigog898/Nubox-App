...
function updateZipButtonState() {
    const hasIds = Array.isArray(tableState.ids) && tableState.ids.length > 0;
...
async function downloadAllPdfsAsZip() {
    ...
    const ids = Array.isArray(tableState.ids)
        ? tableState.ids.filter(Boolean)
        : [];
    ...
    for (let index = 0; index < ids.length; index += 1) {
        ...
        try {
            ...
            zip.file(`documento_${id}.pdf`, pdfBlob);
            successCount += 1;
            updateZipProgress({
                total: ids.length,
...
function setTableRowsFromResponse(responseData) {
    ...
    tableState.ids = Array.from(new Set(rows.map((row) => getRowId(row)).filter(Boolean)));
    tableState.page = 1;
    renderTable();
    updateZipButtonState();
}
